...
// v2.5.0 — maybeSingle para categorias/bairros, busca simplificada
// ============================================================
...
export async function listarConversas() {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      criado_em,
      atualizado_em,
      participante_1:perfis!participante_1(id, nome, avatar_url),
      participante_2:perfis!participante_2(id, nome, avatar_url),
...
export async function buscarMensagens(conversaId, limite = 50) {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      criado_em,
      remetente_id,
      remetente:perfis!remetente_id(id, nome, avatar_url)
    `)
...
/**
 * Cria uma denúncia (report)
 * @param {'post'|'mensagem'|'perfil'|'conversa'} tipo - Tipo do item
 * @param {string} itemId - UUID do item denunciado
 * @param {string} motivo - Motivo (scam_fraude, assedio, spam, etc.)
 * @param {string} [descricao] - Descrição opcional
 */
export async function criarReport(tipo, itemId, motivo, descricao = null) {
  const user = await getUsuarioAtual();
  if (!user) throw new Error('Não autenticado');
...
/**
 * Bloqueia um usuário (usa RPC para segurança)
 * @param {string} bloqueadoId - UUID do usuário a ser bloqueado
 */
...
/**
 * Desbloqueia um usuário (usa RPC para segurança)
 * @param {string} bloqueadoId - UUID do usuário a ser desbloqueado
 */
...
/**
 * Verifica se o usuário atual bloqueou alguém (ou foi bloqueado)
 * @param {string} outroId - UUID do outro usuário
 * @returns {boolean} true se há bloqueio
 */
...
/**
 * Lista IDs de usuários bloqueados pelo usuário atual
