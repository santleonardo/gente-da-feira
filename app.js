 ...
 * Versão: 4.9.0 (Restauração de Moderação: Excluir Posts/Comentários)
 */
...
async function carregarFeed() {
    ...
    posts.forEach(post => {
        ...
        const isDono = currentUid === post.user_id;
        const btnDeletePost = isDono ? `<button onclick="excluirPost('${post.id}')" class="text-[9px] text-red-300 font-bold uppercase hover:text-red-500">Apagar</button>` : '';
        ...
        postEl.innerHTML = `
            <div class="flex items-center gap-4 mb-5">
                <div onclick="verPerfilPublico('${post.user_id}')" class="w-12 h-12 rounded-2xl bg-feira-yellow flex items-center justify-center shadow-md cursor-pointer overflow-hidden">
                    ${avatar}
                </div>
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-tighter">${post.zona}</span>
                        </div>
                        ${btnDeletePost}
                    </div>
                </div>
            ...
            <div id="thread-${post.id}" class="hidden mt-4 space-y-2 pt-4">
                ${post.comments?.map(c => {
                    const isDonoComent = currentUid === c.user_id;
                    const btnDelComent = isDonoComent ? `<button onclick="apagarComentario('${c.id}')" class="ml-2 text-red-300 hover:text-red-500">×</button>` : '';
                    return `<div class="text-xs bg-gray-50 p-3 rounded-2xl flex justify-between">
...
window.verPerfilPublico = async (uid) => {
    ...
    if (p) {
        ...
        if(p.avatar_url) { img.src = p.avatar_url; img.classList.remove('hidden'); fallback.classList.add('hidden'); }
        else { img.classList.add('hidden'); fallback.classList.remove('hidden'); }
        
        const { data: { session } } = await _supabase.auth.getSession();
        const editArea = document.getElementById('edit-button-area');
        if(editArea) editArea.innerHTML = (session && session.user.id === uid) ? `<button onclick="abrirEdicaoPerfil()" class="mt-4 bg-feira-yellow text-feira-marinho px-6 py-2 rounded-xl font-black text-[10px] uppercase">Editar Perfil</button>` : '';
...
window.enviarPost = async () => {
    const { data: { session } } = await _supabase.auth.getSession();
    await _supabase.from('posts').insert({ content: document.getElementById('post-content').value, zona: document.getElementById('post-zona').value, user_id: session.user.id });
    mostrarTela('feed-container'); carregarFeed();
};

window.abrirThreads = (id) => document.getElementById(`thread-${id}`).classList.toggle('hidden');
window.abrirEdicaoPerfil = () => mostrarTela('edit-profile-screen');
window.abrirPostagem = () => mostrarTela('form-post');
window.mudarFeed = () => carregarFeed();
...
window.previewImagem = (event) => {
    ...
    reader.onload = () => {
        const p = document.querySelector('#edit-profile-screen .w-24.h-24');
        p.style.backgroundImage = `url(${reader.result})`; p.style.backgroundSize = 'cover';
    };
