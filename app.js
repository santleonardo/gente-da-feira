...
function renderizarPosts(posts, limpar = false) {
  ...
  posts.forEach(post => {
    container.insertBefore(criarCardPost(post), container.querySelector('#skeleton-loader'));
  });
}
...
function criarCardPost(post) {
  ...
  const destinoPerfil = post.autor?.is_profissional ? 'profissional' : 'usuario';
...
function mostrarErroFeed() {
  ...
  container.innerHTML = `
    <div class="text-center py-12 text-gray-500">
      ...
      <button onclick="carregarFeed(true)" class="mt-3 px-4 py-2 bg-yellow-500 text-white rounded-xl text-sm font-semibold">
        Tentar novamente
      </button>
    </div>`;
}
