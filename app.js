...
// v2.5.0 — Correções: lat/lng bairro, pins mapa com jitter, sync perfil↔mapa, realtime Feira toda, eventos feed
// ============================================================
...
async function carregarBairros() {
  ...
  // "todos" = Feira toda
  if (slugSalvo === 'todos') {
    Estado.bairroAtual = { slug: 'todos', id: null, nome: 'Feira toda', latitude: -12.2547, longitude: -38.9550 };
  } else {
...
function atualizarBairroUI() {
  ...
  if (busca) {
    const placeholder = Estado.bairroAtual.slug === 'todos'
      ? 'O que você precisa em Feira de Santana agora?'
      : `O que você precisa na ${Estado.bairroAtual.nome} agora?`;
...
function abrirModalBairro() {
  ...
  const eTodos = Estado.bairroAtual?.slug === 'todos'
    ...
    <div class="relative bg-white w-full max-w-lg rounded-t-3xl p-6 max-h-[75vh] overflow-y-auto shadow-2xl">
      ...
      <div class="grid grid-cols-2 gap-2" id="lista-bairros-modal">
        <button
          ...
          class="bairro-opcao p-3 text-sm font-medium text-left rounded-xl border-2 transition-all ${eTodos}">
          🏙️ Feira toda
        </button>
        ${Estado.bairros
          .map(
  ...
  modal.querySelectorAll('.bairro-opcao').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (slug === 'todos') {
        ...
        // Buscar lat/lng do bairro para o mapa
        const bairroCompleto = Estado.bairros.find(b => b.slug === slug);
        Estado.bairroAtual = {
          slug,
          id,
          nome,
          latitude: bairroCompleto?.latitude || -12.2547,
          longitude: bairroCompleto?.longitude || -38.9550,
        };
      }
      ...
      await carregarFeed(true);
      atualizarBadgesGrid();
      mostrarToast(slug === 'todos' ? 'Mostrando toda Feira de Santana 🏙️' : `Bairro alterado para ${nome} 📍`);

      // Se o mapa está aberto, recarregar também
      if (Estado.telaAtual === 'mapa') {
        const mapaNome = document.getElementById('mapa-bairro-nome');
        if (mapaNome) mapaNome.textContent = Estado.bairroAtual.nome;
        if (mapaInstancia) {
          mapaInstancia.setView(
            [Estado.bairroAtual.latitude || -12.2547, Estado.bairroAtual.longitude || -38.9550],
            Estado.bairroAtual.slug === 'todos' ? 13 : 15
          );
        }
        mostrarBairrosNoMapa();
        carregarPostsNoMapa();
      }
    });
  });
...
async function abrirModalPublicar() {
  ...
  modal.querySelector('#btn-confirmar-publicar').addEventListener('click', async () => {
    ...
    if (!titulo) { mostrarErroModal(erroEl, 'Preencha o título'); return; }
    if (!descricao) { mostrarErroModal(erroEl, 'Preencha a descrição'); return; }
    if (!Estado.bairroAtual?.id) { mostrarErroModal(erroEl, 'Selecione um bairro específico para publicar. Com "Feira toda" você só visualiza.'); return; }
...
function abrirTelaMapa() {
  ...
  // Atualizar nome do bairro no header
  const mapaNome = document.getElementById('mapa-bairro-nome');
  if (mapaNome) mapaNome.textContent = Estado.bairroAtual?.nome || 'Feira de Santana';
  ...
  setTimeout(() => {
    ...
    // Mostrar marcadores de bairros clicáveis
    mostrarBairrosNoMapa();
...
function mostrarBairrosNoMapa() {
  if (!mapaInstancia) return;

  // Limpar marcadores antigos de bairros
  marcadoresBairro.forEach(m => mapaInstancia.removeLayer(m));
  marcadoresBairro = [];

  // Mostrar todos os bairros com marcadores azuis
  Estado.bairros.forEach(b => {
    if (!b.latitude || !b.longitude) return;

    const isAtual = Estado.bairroAtual?.slug === b.slug;
    const marker = L.circleMarker([b.latitude, b.longitude], {
      radius: isAtual ? 14 : 8,
      fillColor: isAtual ? '#F59E0B' : '#3B82F6',
      color: '#fff',
      weight: isAtual ? 3 : 2,
      opacity: 1,
      fillOpacity: 0.7,
    ...
    marker.bindPopup(`
      <div style="min-width: 160px; font-family: system-ui; text-align: center;">
        <strong style="font-size: 14px;">${esc(b.nome)}</strong>
        <p style="font-size: 11px; color: #666; margin-top: 4px;">${isAtual ? 'Bairro selecionado' : 'Toque para ver posts'}</p>
        ${!isAtual ? `<button class="mapa-selecionar-bairro-btn" data-bairro-slug="${escAttr(b.slug)}" data-bairro-id="${escAttr(b.id)}" data-bairro-nome="${escAttr(b.nome)}" data-bairro-lat="${b.latitude}" data-bairro-lng="${b.longitude}" style="margin-top: 6px; padding: 4px 12px; background: #F59E0B; color: white; border: none; border-radius: 8px; font-size: 12px; cursor: pointer;">Ver posts aqui</button>` : ''}
      </div>
    `);

    // Handler para o botão dentro do popup
    marker.on('popupopen', () => {
      setTimeout(() => {
        const popupEl = marker.getPopup()?.getElement();
        if (popupEl) {
          const btn = popupEl.querySelector('.mapa-selecionar-bairro-btn');
          if (btn) {
            btn.addEventListener('click', async () => {
              const bSlug = btn.dataset.bairroSlug;
              const bId = btn.dataset.bairroId;
              const bNome = btn.dataset.bairroNome;
              const bLat = parseFloat(btn.dataset.bairroLat);
              const bLng = parseFloat(btn.dataset.bairroLng);

              // Atualizar bairro atual
              Estado.bairroAtual = { slug: bSlug, id: bId, nome: bNome, latitude: bLat, longitude: bLng };
              localStorage.setItem('gdf_bairro_slug', bSlug);
              atualizarBairroUI();

              // Atualizar header do mapa
              const mapaNome = document.getElementById('mapa-bairro-nome');
              if (mapaNome) mapaNome.textContent = bNome;

              // Mover mapa e recarregar
              mapaInstancia.setView([bLat, bLng], 15);
              mostrarBairrosNoMapa();
              await carregarPostsNoMapa();
              await carregarFeed(true);
              atualizarBadgesGrid();
              mostrarToast(`Bairro alterado para ${bNome} 📍`);
            });
          }
        }
      }, 50);
...
async function carregarPostsNoMapa() {
  ...
  try {
    const mapaBusca = document.getElementById('mapa-busca');
    const buscaTexto = mapaBusca?.value?.trim();

    const posts = await buscarPosts({
      bairroSlug: Estado.bairroAtual?.slug || 'todos',
      busca: buscaTexto || undefined,
      limite: 50,
    });

    // Agrupar posts por bairro para calcular jitter diferente para cada um
    const postsPorBairro = {};
    posts.forEach(post => {
      const bairroKey = post.bairro?.id || 'sem-bairro';
      if (!postsPorBairro[bairroKey]) postsPorBairro[bairroKey] = [];
      postsPorBairro[bairroKey].push(post);
    });

    // Gerar jitter pseudo-aleatório baseado no índice do post
    // Espalha os pins em círculo ao redor do centro do bairro
    for (const [bairroKey, bairroPosts] of Object.entries(postsPorBairro)) {
    const centroLat = parseFloat(bairroPosts[0]?.bairro?.latitude) || -12.2547;
    const centroLng = parseFloat(bairroPosts[0]?.bairro?.longitude) || -38.9550;

    bairroPosts.forEach((post, idx) => {
      const lat = post.latitude ? parseFloat(post.latitude) : centroLat;
      const lng = post.longitude ? parseFloat(post.longitude) : centroLng;
      if (!lat || !lng) return;

      // Se o post tem coordenadas próprias, usar sem jitter
      // Se usa as do bairro, adicionar jitter para não empilhar
      let finalLat = lat;
      let finalLng = lng;

      if (!post.latitude && bairroPosts.length > 1) {
        // Espalhar em espiral: ~0.001 grau (~100m) de raio
        const angulo = (idx / bairroPosts.length) * 2 * Math.PI;
        const raio = 0.001 + (idx * 0.0003);
        finalLat = lat + Math.cos(angulo) * raio;
        finalLng = lng + Math.sin(angulo) * raio;
      }
      ...
      marker.bindPopup(`
        <div style="min-width: 200px; font-family: system-ui;">
          ...
          <p style="font-size: 11px; color: #999;">${esc(post.bairro?.nome || '')} • ${esc(post.autor?.nome || 'Anônimo')} • ${formatarTempo(post.criado_em)}</p>
        </div>
      `);
...
function iniciarRealtime() {
  ...
  // Se "Feira toda", escutar sem filtro de bairro (todos os posts)
  // Se bairro específico, escutar apenas esse bairro
  const bairroId = Estado.bairroAtual?.id;
  const ehFeiraToda = Estado.bairroAtual?.slug === 'todos';

  if (!bairroId && !ehFeiraToda) return;

  if (ehFeiraToda) {
    // Escutar qualquer novo post (sem filtro de bairro)
    const channel = supabase
      .channel('novos-posts-feira-toda')
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'posts' },
        () => {
          mostrarToast('📢 Nova publicação em Feira de Santana!');
          carregarFeed(true);
        }
      )
      .subscribe();
    Estado.unsubscribeRealtime = () => supabase.removeChannel(channel);
  } else {
    Estado.unsubscribeRealtime = escutarNovosPosts(bairroId, (novoPost) => {
...
async function atualizarBadgesGrid() {
  ...
  try {
    ...
    for (const cat of categorias) {
      // Se "Feira toda", contar todos; senão, filtrar por bairro
      let query = supabase
        .from('posts')
        .select('*', { count: 'exact', head: true })
        .eq('ativo', true)
      ...
      if (Estado.bairroAtual.slug !== 'todos' && Estado.bairroAtual.id) {
...
function setupEventListeners() {
  ...
  // Mapa — busca de serviços (com debounce e botão limpar)
  let mapaBuscaTimeout = null;
  const mapaBuscaInput = document.getElementById('mapa-busca');
  const mapaBuscaLimpar = document.getElementById('mapa-busca-limpar');

  mapaBuscaInput?.addEventListener('input', (e) => {
    const texto = e.target.value;
    // Mostrar/esconder botão limpar
    if (mapaBuscaLimpar) mapaBuscaLimpar.classList.toggle('hidden', !texto);

    clearTimeout(mapaBuscaTimeout);
    mapaBuscaTimeout = setTimeout(() => {
      carregarPostsNoMapa();
    }, 400);
  });

  mapaBuscaLimpar?.addEventListener('click', () => {
    if (mapaBuscaInput) mapaBuscaInput.value = '';
    if (mapaBuscaLimpar) mapaBuscaLimpar.classList.add('hidden');
    carregarPostsNoMapa();
  });
...
function abrirModalPerfil() {
  ...
  modal.querySelector('#btn-salvar-perfil').addEventListener('click', async () => {
    ...
    try {
      ...
      // Se trocou de bairro no perfil, atualizar o feed, mapa e realtime
      if (bairroId) {
        const bairroNovo = Estado.bairros.find(b => b.id === bairroId);
        if (bairroNovo && (!Estado.bairroAtual || Estado.bairroAtual.id !== bairroNovo.id)) {
          Estado.bairroAtual = {
            ...bairroNovo,
            latitude: bairroNovo.latitude || -12.2547,
            longitude: bairroNovo.longitude || -38.9550,
          };
          localStorage.setItem('gdf_bairro_slug', bairroNovo.slug);
          atualizarBairroUI();
          await carregarFeed(true);
          atualizarBadgesGrid();

          // Reiniciar realtime com o novo bairro
          if (Estado.unsubscribeRealtime) Estado.unsubscribeRealtime();
          iniciarRealtime();
          ...
          if (Estado.telaAtual === 'mapa') {
            const mapaNome = document.getElementById('mapa-bairro-nome');
            if (mapaNome) mapaNome.textContent = bairroNovo.nome;
            if (mapaInstancia) {
              mapaInstancia.setView(
                [bairroNovo.latitude || -12.2547, bairroNovo.longitude || -38.9550],
                15
              );
            }
            mostrarBairrosNoMapa();
