...
async function abrirDetalhePost(postId) {
  ...
  modal.querySelector('#btn-chat-com-autor')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const outroNome = conversa.participante_1?.id === Estado.usuario?.id
        ? conversa.participante_2?.nome
        : conversa.participante_1?.nome;
      modal.remove();
      await abrirTelaChat(conversa.id, outroNome || 'Usuário');
    } catch (err) {
...
function abrirTelaMapa() {
  ...
  const mapaBairroNome = document.getElementById('mapa-bairro-nome');
  if (mapaBairroNome) mapaBairroNome.textContent = Estado.bairroAtual?.nome || 'Feira de Santana';

  // Limpar busca
  const buscaMapa = document.getElementById('busca-mapa');
  if (buscaMapa) buscaMapa.value = '';
  const btnLimpar = document.getElementById('busca-mapa-limpar');
...
async function carregarPostsNoMapa() {
  ...
  try {
    ...
    // Agrupar posts sem coordenadas próprias para aplicar jitter
    const postsSemCoordenadas = [];
    const postsComCoordenadas = [];

    posts.forEach(post => {
      if (post.latitude && post.longitude) {
        postsComCoordenadas.push(post);
      } else if (post.bairro?.latitude && post.bairro?.longitude) {
        postsSemCoordenadas.push(post);
      }
    });

    // Posts com coordenadas próprias — sem jitter
    postsComCoordenadas.forEach(post => {
    ...
    if (Estado.bairroAtual?.latitude && Estado.bairroAtual?.longitude) {
      ...
      bounds.extend([Estado.bairroAtual.latitude, Estado.bairroAtual.longitude]);
    }

    // Ajustar zoom para mostrar todos os marcadores
    if (bounds.isValid()) {
      mapaInstancia.fitBounds(bounds.pad(0.2), { maxZoom: 16 });
    }
...
// Pesquisa no mapa com debounce
let debounceMapa = null;
document.addEventListener('DOMContentLoaded', () => {
  const inputBusca = document.getElementById('busca-mapa');
  const btnLimpar = document.getElementById('busca-mapa-limpar');
  ...
  inputBusca.addEventListener('input', () => {
    ...
    debounceMapa = setTimeout(() => {
      if (!mapaInstancia) return;

      // Se termo vazio, recarregar todos os posts
      if (!termo) {
        carregarPostsNoMapa();
        return;
      }

      // Filtrar marcadores visíveis pelo termo
      marcadores.forEach(m => {
        const popupContent = m.getPopup()?.getContent() || '';
        const match = popupContent.toLowerCase().includes(termo.toLowerCase());
        if (match) {
          if (!mapaInstancia.hasLayer(m)) m.addTo(mapaInstancia);
        } else {
          if (mapaInstancia.hasLayer(m)) mapaInstancia.removeLayer(m);
        }
      });
    }, 400);
  });
  ...
  btnLimpar?.addEventListener('click', () => {
    ...
    if (mapaInstancia) carregarPostsNoMapa();
  });
});
...
// ============================================================
async function abrirTelaChat(conversaIdParaAbrir = null, nomeOutro = null) {
  if (!Estado.usuario) {
  ...
  if (conversaIdParaAbrir) {
    // Abrir diretamente numa conversa específica (vindo do post ou perfil)
    const mensagensContainer = document.getElementById('chat-mensagens');
    if (mensagensContainer) mensagensContainer.innerHTML = '<div class="p-6 text-center text-gray-400">Carregando...</div>';
    await abrirConversa(conversaIdParaAbrir, nomeOutro || 'Usuário');
  } else {
...
async function carregarListaConversas() {
  ...
  document.getElementById('chat-header-lista')?.classList.remove('hidden');
  ...
  modal.querySelector('#btn-chat-perfil')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const outroNome = conversa.participante_1?.id === Estado.usuario?.id
        ...
        : conversa.participante_1?.nome;
      modal.remove();
      await abrirTelaChat(conversa.id, perfil?.nome || outroNome || 'Usuário');
    } catch (err) {
      mostrarToast('Erro ao abrir chat', 'erro');
      btn.textContent = 'Enviar Mensagem';
      btn.disabled = false;
