...
// ─── Author profile column sets (for joined profiles in posts, comments, etc.)
// These are the ONLY columns that should appear when a user's profile
// is shown as an "author" of content (posts, comments, reactions, etc.) ──

/** Author columns when neighborhood might be visible */
export const AUTHOR_PROFILE_COLUMNS_WITH_NEIGHBORHOOD = [
  "id", "display_name", "username", "avatar_url", "neighborhood",
] as const;

/** Author columns WITHOUT neighborhood — used when hide_neighborhood may apply */
export const AUTHOR_PROFILE_COLUMNS = [
  "id", "display_name", "username", "avatar_url",
] as const;

/** Follow list profile columns — minimal for follower/following lists */
export const FOLLOW_LIST_PROFILE_COLUMNS = [
  "id", "display_name", "username", "avatar_url", "bio",
] as const;

/** Follow list profile columns WITHOUT bio (for requests from non-owners) */
export const FOLLOW_LIST_PROFILE_COLUMNS_MINIMAL = [
  "id", "display_name", "username", "avatar_url",
] as const;
...
/**
 * Strip `neighborhood` from a profile object if the target user has
 * `hide_neighborhood` enabled and the viewer is NOT the profile owner.
 */
export function filterNeighborhood(
  profile: Record<string, any>,
  ctx: PrivacyContext
): Record<string, any> {
  if (ctx.isOwnProfile) return profile;
  if (!ctx.privacy.hide_neighborhood) return profile;
  // Replace with null — never send the actual value
  const { neighborhood, ...rest } = profile;
  return { ...rest, neighborhood: null };
}

/**
 * Build a safe author profile object for use in posts, comments, reactions, etc.
 * This is the PRIMARY function to use whenever a profile is joined as "author".
 * It automatically handles hide_neighborhood.
 */
export function buildSafeAuthorProfile(
  authorProfile: Record<string, any> | null | undefined,
  viewerId: string | null,
  targetPrivacyFlags?: Partial<PrivacyFlags>
): Record<string, any> | null {
  if (!authorProfile) return null;

  // If we don't have privacy flags, return the profile as-is
  // (the API should ideally fetch privacy flags, but some lightweight
  // contexts like comment authors don't need the full check)
  if (!targetPrivacyFlags) {
    return {
      id: authorProfile.id,
  ...
  const isOwnProfile = viewerId === authorProfile.id;
  const hideNeighborhood = targetPrivacyFlags.hide_neighborhood === true;
  ...
  if (!isOwnProfile && hideNeighborhood) {
    result.neighborhood = null;
  } else if (authorProfile.neighborhood !== undefined) {
    result.neighborhood = authorProfile.neighborhood;
  }
