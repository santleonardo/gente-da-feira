...
function RoomChat({ room, onBack, onRefreshRooms, openUserProfile }: { room: any; onBack: () => void; onRefreshRooms: () => void; openUserProfile?: (userId: string) => void }) {
   ...
   * Caso contrário o browser NUNCA mostra o prompt do microfone.
   */
  const startAudioRecording = () => {
    ...
    // 3) Só agora pode fechar menu / setState
    setAttachMenuOpen(false);
          ...
          <>
            ...
            {!sendingMedia && (
              <div className="flex items-end gap-1.5 sm:gap-2 max-w-3xl mx-auto w-full min-w-0">
                {/* Microfone direto — mesmo gesto do toque chama getUserMedia */}
                <button
                  type="button"
                  onClick={(e) => {
                    e.preventDefault();
              ...
              {[
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    // Action no mesmo gesto (getUserMedia / file input)
                    item.action();
                  }}
