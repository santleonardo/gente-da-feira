...
export async function GET(req: NextRequest) {
  try {
    ...
    const { message, status } = safeErrorResponse(error, 500, "[profile-photos/comments GET]");
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { message, status } = safeErrorResponse(error, 500, "[profile-photos/comments POST]");
    return NextResponse.json({ error: message }, { status });
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await idempotencyFail(req);
    const { message, status } = safeErrorResponse(error, 500, "[profile-photos/comments DELETE]");
    return NextResponse.json({ error: message }, { status });
  }
}